	...
	switch(command) {
		case "ttt":
            ...
            if (typeof users[playerName] != 'undefined') {
                message.sendMessage("The player is already in a game");
                return;
            }

            if (playerName == "TicTacToe") {
                message.sendMessage("Meh! you were to easy for me :wink:");
                return;
            }

            if (!client.users.exists('username', playerName)) {
                message.sendMessage("Player could not found");
                return;
            }

            if (playerName == message.author.username) {
                message.sendMessage("Are you stupid? You cant play against you");
                return;
            }

            if (typeof lastStartedGames[userName] != 'undefined') {
                message.sendMessage("You can start every 30 seconds a game");
                return;
            }
            ...
            message.sendMessage("Request to player was sended. You can end the game with ``!end``");
		...
		case "ap":
			if (typeof users[userName] == 'undefined') {
                message.sendMessage("You dont have a game request");
                return;
            }
            ...
            if (game.requestedFrom.username != userName && !games[gameId].started) {
                ...
                message.sendMessage("Game begings Type !pl [A-C][1-3]\n**[A-C]** horizontal\n**[1-3]** vertical.\nExample A1 is top left");
            } else {
                message.sendMessage("The rival must accept you faggot! :wink:");
                return;
            }
		break;
		case "pl":
            ...
            if (typeof users[userName] == 'undefined') {
                message.sendMessage("You don\'t have a game request");
                return;
            }
            ...
            if (games[gameId].placeUser.username != userName) {
                message.sendMessage("The rival plays next");
                return;
            }
            ...
            if (checkPlace(gameId, place)) {
                ...
                message.sendMessage("The rival plays now");
                ...
                if (Object.keys(games[gameId].grid).length == 9) {
                    ...
                    message.sendMessage("Game finished. All fields are used");
                } else {
                    ...
                    if (winner != "") {
                        ...
                        message.sendMessage("Game finished. " + winnerName + " has won");
                    }
                }
            } else {
                message.sendMessage("Wrong input value or the place is used");
            }
		break;
		case "end":
            if (typeof users[userName] == 'undefined') {
            ...
            if (Object.keys(games[gameId].grid).length >= 3) {
                message.sendMessage("You cant end the game, if 3 or more fields are used");
                return;
            }
		...
		break;
		case "ping":
			message.sendMessage('Your expecting me to say Pong, right?');
		break;
		case "ding":
			message.sendMessage('Dong!');
		break;
		case "cmds":
			message.sendMessage('Visit the documentation on our site for more info: http://docs.mikuchan.me/commands');
			break;
		case "icup":
			message.sendMessage('Ha ha. Very funny. ***(not)***.');
			break;
		case "fetchrole":
			if(message.member.roles.has('366256286922178560')) {
				message.sendMessage('Sorry, you already have the needed rank.');
			} else {
				member.addRole('366256286922178560').catch(console.error);
				message.sendMessage('Fine. Take your stupid role.');
			}
			break;
		case "about":
		...
		case "radio":
			if (message.member.voiceChannel) {
				...
				message.sendMessage('Uh.. That\'s not supposed to happen');
			}
			break;
		case "google":
			let query = args[0];
		...
		case "reboot":
			if(message.author.id == config.ownerID) {
				...
				message.sendMessage('Heh, nice try douche. I only listen to my creators here.');
			}
			break;
	}
});
